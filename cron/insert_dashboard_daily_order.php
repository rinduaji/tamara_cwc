...
$date = date("Y-m-d");
// $date = date("Y-m-d",strtotime('2022-11-11'));
...
$total_cek = mysqli_num_rows($result_cek);
    ...
    $query = "
        ...
        f.jumlah_call_agree, f.total_call_agree, h.jumlah_not_agree, h.total_not_agree, i.jumlah_follow_up, i.total_follow_up,
        g.jumlah_revenue, g.total_revenue, (g.jumlah_revenue / c.jumlah_agent) as rpa, (f.jumlah_call_agree / c.jumlah_agent) as ppa
        FROM
        (
        ...
        (
            ...
            FROM
            (	
                SELECT DATE(tgl) as tanggal, UPPER(area) as area, jenis, COUNT(login) as jumlah_not_agree FROM app_tam_data2 WHERE status= 'Contacted' AND 
                kategori = 'Not Agree' GROUP BY DATE(tgl), area, jenis ORDER BY DATE(tgl), area, jenis ASC
            ) as a
            INNER JOIN
            (
                SELECT DATE(tgl) as tanggal, COUNT(login) as total_not_agree FROM app_tam_data2 WHERE status= 'Contacted' AND 
        ...
        (
            ...
            FROM
            (	
                SELECT DATE(tgl) as tanggal, UPPER(area) as area, jenis, COUNT(*) as jumlah_follow_up FROM app_tam_data2 WHERE status= 'Contacted' AND 
                UPPER(kategori) = 'FOLLOW UP' GROUP BY DATE(tgl), area, jenis ORDER BY DATE(tgl), area, jenis  ASC
            ) as a
            INNER JOIN
            (
                SELECT DATE(tgl) as tanggal, COUNT(login) as total_follow_up FROM app_tam_data2 WHERE status= 'Contacted' AND 
                UPPER(kategori) = 'FOLLOW UP' GROUP BY DATE(tgl) ORDER BY DATE(tgl) ASC
            ) as b
            ON a.tanggal = b.tanggal
        ) as i
    ...
    $allData = "INSERT INTO app_tam_dashboard (tanggal, area, jenis, jumlah_agent, total_agent, jumlah_call, total_call, jumlah_call_contacted, total_call_contacted, 
                jumlah_call_agree, total_call_agree, jumlah_not_agree, total_not_agree, jumlah_follow_up, total_follow_up, jumlah_revenue, total_revenue, rpa, ppa) VALUES";
    $area = "";
    $tgl = "";
    $total_rpa = array();
    $total_ppa = array();
...
if(isset($total_data)) {  
    if($total_data == 0 ) {    
        ...
        $query1 = "
        SELECT * FROM (
        SELECT b.tanggal, b.login, b.nama_agent, b.area, b.revenue, b.rank
        FROM
        (
            SELECT a.tanggal, a.login, a.nama_agent, a.area, a.revenue,
            ...
            (
                    ...
                    GROUP BY login ORDER BY tanggal, area, jenis, revenue, login DESC
...
print_r($total1);
        ...
        while ($row1 = mysqli_fetch_assoc($result1)) {
            ...
            $sql = "UPDATE app_tam_dashboard SET total_rpa='".$total_rpa[$row1['area']]."', total_ppa='".$total_ppa[$row1['area']]."', best_agent = '".$row1['nama_agent'].
                    "', revenue_tertinggi = '".$row1['revenue']."', lup = '".$lup."' WHERE tanggal = '$date' AND area = '".$row1['area']."'";
        ...
        SELECT * FROM (
                    ...
                    (
                        ...
                        (
                                SELECT DATE(tgl) as tanggal, login, nama_agent, UPPER(area) as area, jenis,  COUNT(jenis) as tot_jenis, SUM(add_on_tsel) as revenue
                                FROM app_tam_data2 
                                WHERE status = 'Contacted' AND kategori = 'Agree' AND follow = '6' AND valid = 'Valid' AND MONTH(tgl) = MONTH('$date') 
                                GROUP BY login ORDER BY tanggal, area, jenis, revenue, login DESC
                        ) as a, (SELECT @curr_count:=0) r, (SELECT @curr_value:=0) q
                        ORDER BY a.area ASC, a.revenue DESC
                    ) as b 
        ...
        while ($row_month = mysqli_fetch_assoc($result_best_month)) {  
            $sql = "UPDATE app_tam_dashboard SET best_agent_month = '".$row_month['nama_agent'].
                "', revenue_tertinggi_month = '".$row_month['revenue']."', lup = '".$lup."' WHERE tanggal = '$date' AND area = '".$row_month['area']."'";
            $hasil_update = mysqli_query($con, $sql);
        }    
        ...
        (
            ...
            (
                    SELECT DATE(tgl) as tanggal, login, nama_agent, UPPER(area) as area, jenis,  COUNT(jenis) as tot_jenis, SUM(add_on_tsel) as revenue
                    FROM app_tam_data2 
                    WHERE status = 'Contacted' AND kategori = 'Agree' AND follow = '6' AND valid = 'Valid' AND YEARWEEK(tgl) = YEARWEEK('$date') 
                    GROUP BY login ORDER BY tanggal, area, jenis, revenue, login DESC
            ) as a, (SELECT @curr_count:=0) r, (SELECT @curr_value:=0) q
            ORDER BY a.area ASC, a.revenue DESC
        ) as b 
        ...
        while ($row_week = mysqli_fetch_assoc($result_best_week)) {  
            $sql = "UPDATE app_tam_dashboard SET best_agent_week = '".$row_week['nama_agent'].
                "', revenue_tertinggi_week = '".$row_week['revenue']."', lup = '".$lup."' WHERE tanggal = '$date' AND area = '".$row_week['area']."'";
            $hasil_update = mysqli_query($con, $sql);
        }  
...
print_r($total1);
        ...
        while ($row1 = mysqli_fetch_assoc($result_low_agent)) {
            // print_r($row1);
            $sql = "UPDATE app_tam_dashboard SET total_rpa='".$total_rpa[$row1['area']]."', total_ppa='".$total_ppa[$row1['area']]."', low_agent = '".$row1['nama_agent'].
                    "', revenue_low = '".$row1['revenue']."', lup = '".$lup."' WHERE tanggal = '$date' AND area = '".$row1['area']."'";
            $hasil_update = mysqli_query($con, $sql);
        ...
        while ($row_month = mysqli_fetch_assoc($result_low_month)) {  
            $sql = "UPDATE app_tam_dashboard SET low_agent_month = '".$row_month['nama_agent'].
                "', revenue_low_month = '".$row_month['revenue']."', lup = '".$lup."' WHERE tanggal = '$date' AND area = '".$row_month['area']."'";
            $hasil_update = mysqli_query($con, $sql);
        }    
        ...
        while ($row_week = mysqli_fetch_assoc($result_low_week)) {  
            $sql = "UPDATE app_tam_dashboard SET low_agent_week = '".$row_week['nama_agent'].
                "', revenue_low_week = '".$row_week['revenue']."', lup = '".$lup."' WHERE tanggal = '$date' AND area = '".$row_week['area']."'";
            $hasil_update = mysqli_query($con, $sql);
        }
    }
}
