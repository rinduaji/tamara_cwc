...
if (isset($_POST['Save']) ){
	...
	if($status=="Not Contacted" || $status=="Follow Up Recall")
		{   
			...
			echo "<div class=\"alert alert-danger\">Mohon periksa kembali! Input Anda ada yang belum di isi !</div>";
				...
				$sql="INSERT INTO app_tam_data2 (tgl,login,nama_dm,tlp,`status`,reg,fastel,penerima,jenis,kategori,reason,Ket,follow,upd_qco,valid,input,add_on_tsel,nama_agent,area,paket_steaming,
				speed_asal,speed_tujuan,`param`,internet,ott1,ott2,paket_harga,diskon,total_harga_diskon, hssp_type, recom_speed, blast, speed_tertinggi, status_hd) VALUES ('$tgl','$login','$nama_dm','$tlp','$status','$reg','$fastel','$penerima','$jenis','$kategori','$reason',
				'$ket','5','$upd_qco','Not Valid','Recall','$add','$nama','$area','$paket_streaming','$speed_asal','$speed_tujuan','$param','$internet','$ott1','$ott2','$paket_harga','$diskon',
				'$total_harga_diskon','$hssp_type','$recom_speed','$blast','$speed_tertinggi','0')";
	...
	else if($status=="Contacted")
	{
		...
		if ( $fastel==""  or $login=="" or $nama_dm=="" or $reg=="" or $tlp=="" or $status=="" or $jenis=="" or $kategori=="" or $reason=="" or $ket=="" or $follow=="")
		{	
		echo "<div class=\"alert alert-danger\">Mohon periksa kembali! Input Anda ada yang belum di isi !</div>";
		}else{  
			$sql="UPDATE `app_tam_data2` SET `follow`='1'  WHERE (`id`='$id')";
			...
			$sql="INSERT INTO app_tam_data2 (tgl,login,nama_dm,tlp,`status`,reg,fastel,penerima,jenis,kategori,reason,Ket,follow,upd_qco,valid,input,add_on_tsel,nama_agent,area,
			...
			'$ket','$fol','$upd_qco','Not Valid','Recall','$add','$nama','$area','$paket_streaming','$speed_asal','$speed_tujuan','$param','$internet','$ott1','$ott2','$paket_harga',
			'$diskon','$total_harga_diskon','$hssp_type','$recom_speed','$blast','$speed_tertinggi','0')";
			$query = mysqli_query($con,$sql);
			//echo $sql;
			clearstatcache();
			$location="list_recall.php";		
		...
		<div class="content">
			...
			<div class="col-md-12">
					<div class="card card-plain">
						<div class="header">
							<h4 class="title">CWC <small>Form Input Recall Netflix<?php echo $nama; ?></small></h4>	
						</div>
				
				<form id='input' name="demoform" action='form_recall_netflix.php?id=<?php echo "$id";?>' method='post' accept-charset='UTF-8'>
				<?php
					...
					<div class="col-md-4">
								...
								<div class="col-md-4">
									<div class="form-group">
										<label class="control-label" style="color:#008B8B">Speed Loyalti</label>
										<input class="form-control" type="text" name="speed_loyalty" id="speed_loyalty" value="<?php echo $data_wo['speed_loyalty']; ?>" placeholder="Speed Loyalty" required  readonly/>
									</div>
								</div>
								<div class="col-md-4">
									<div class="form-group">
