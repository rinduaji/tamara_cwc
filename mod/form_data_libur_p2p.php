...
if (isset($tanggal)) {
	$query = "SELECT a.id,a.tgl,a.login,c.user2,a.upd_qco,c.user5,c.user1,d.name,a.msisdn,a.type_p2p FROM app_tam_p2p a INNER JOIN app_tam_libur b ON a.upd_qco=b.username INNER JOIN cc147_main_users_extended c ON a.login  = c.user1 INNER JOIN cc147_main_users d ON a.upd_qco=d.username WHERE b.tanggal='$tanggal1' and a.tgl BETWEEN '$tanggaldate1' AND '$tanggaldate2' and a.status='Contacted' and a.kategori='Agree' and a.valid='Not Valid' and a.follow='2' ORDER BY a.tgl DESC";
} else {
	$query = "SELECT a.id,a.tgl,a.login,c.user2,a.upd_qco,c.user5,c.user1,d.name,a.msisdn,a.type_p2p FROM app_tam_p2p a INNER JOIN app_tam_libur b ON a.upd_qco=b.username INNER JOIN cc147_main_users_extended c ON a.login  = c.user1 INNER JOIN cc147_main_users d ON a.upd_qco=d.username WHERE b.tanggal='$tanggal2' and a.tgl BETWEEN '$tanggaldate3' AND '$tanggaldate4' and a.status='Contacted' and a.kategori='Agree' and a.valid='Not Valid' and a.follow='2' ORDER BY a.tgl DESC";
}

$pick_data_bebas = '';
$query_cek_kirim_qco = "SELECT * FROM app_tam_p2p WHERE id='$id'";
...
while ($data_cek_kirim_qco = mysqli_fetch_array($hasil_cek_kirim_qco)) {
	$pick_data_bebas = $data_cek_kirim_qco['pick_data_bebas'];
}


if($pick_data_bebas == '1') {
	echo "<script>alert('data sudah di kirim oleh QA yang lain')</script>";
}
else {
	if ($id != "") {
		$query_edit = "UPDATE app_tam_p2p SET upd_qco='$username_login', pick_data_bebas='1' WHERE id='$id'";
				...
				<div class="card card-plain">
					...
					<div>
						<div class="row row-centered">
							...
							<div align="center" class="col-md-4">
								<form id='input' name="demoform" action='form_data_libur_p2p.php' method='post' accept-charset='UTF-8'>
									<tr>
									<tr>
							...
							<div class="row">
								<div class="col-md-12">
									<div class="card" style="margin-top: 20px;">
														...
														<div class="modal fade" id="sendQCO<?= $data['id'] ?>" tabindex="-1" role="dialog" aria-labelledby="Form Libur" aria-hidden="true" data-backdrop="false">
															<div class="modal-dialog" role="document">
																<div class="modal-content">
																	...
																	<form action="form_data_libur_p2p.php" method="POST" id="FormRekom_qa">
										...
										<table>
											<?php
											//$query   = "SELECT COUNT(*) AS jumData FROM app_tam_p2p as a INNER JOIN cc147_main_users_extended AS b ON a.login = b.user1 $whr ";
											//echo $query;
