...
if(isset($id)) {
    ...
    if ($status_ubah == "Edit") {
        ...
        add_on_tsel = '$add_on_tsel', id_return_tl = '$return_tl' WHERE id='$id'";
    ...
    // if($tgl != "" AND $fastel != "") {
        ...
        `reason`, `penerima`, `ket`, `follow`, `upd_qco`, `rec_qco`, `tgl_qco1`, `valid`, `input`, `add_on_tsel`, `status_history`, `upd`, `nama_upd`, `jabatan_upd`, `tgl_input`, `id_return_tl`) VALUES ('$tgl', '$login', '$fastel', '$nama_dm', '$tlp', '$reg', '$jenis', '$status', '$kategori', 
        '$reason', '$penerima', '$ket', '$follow', '$upd_qco', '$rec_qco', '$tgl_qco1', '$valid', '$input', '$add_on_tsel', '$ubah_data','$username','$nama','$jb','$date_now', '$return_tl')";
        mysqli_query($con, $query);
    // }
}
// if($jb=="Tabber"){
