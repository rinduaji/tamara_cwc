...
if (isset($_POST['Save']) ){
	...
	if($cek_kode_voucher == "double") {
		...
		if($kategori=="Not Contacted" || $kategori=="Follow Up Recall")
			{   
				if ($no_inet == ""  or $no_telp == ""  or $email == ""  or $login == "" or $nama_plg == "" or $no_telp == "" or $kategori == "" or $sub_kat == "" or $kendala == "" ) {
					...
					// $sql="INSERT INTO app_tam_hvc (tgl,login,nama_dm,tlp,`status`,reg,fastel,penerima,jenis,kategori,reason,Ket,follow,upd_qco,valid,input,add_on_tsel,nama_agent,area) VALUES ('$tgl','$login','$nama_dm','$tlp','$status','$reg','$fastel','$penerima','$jenis','$kategori','$reason','$kets','5','$upd_qco','Not Valid','Recall','$add','$nama','$area')";
					$sql = "INSERT INTO app_tam_hvc (tgl,login,nama_agent,area,no_telp,no_inet,ncli,nama_plg,kelamin,hp_utama,layanan,symptom,kategori_hvc,nama_pemilik,relasi,hp_utama2,hp_lain,email,alamat,kota,
								...
								'$email','$alamat','$kota','$saran','$kategori','$sub_kat','$status_call','$ket','$kode_voucher','$tgl_in_voucher','$nama_voucher','$kendala','5','Not Valid','Recall','$channel','$hobbyFav','$jenis_call','$reason_call','$convertRupiah','$upd_qco')";
		...
		{
			if ($sub_kat == "Follow UP") { $fol = 5; $input ="Recall";} else { $fol = 2; $input ="Recall";}
			if ($no_inet == ""  or $no_telp == ""  or $email == ""  or $login == "" or $nama_plg == "" or $no_telp == "" or $kategori == "" or $sub_kat == "" or $kendala == "" ) {
				echo "<div class=\"alert alert-danger\">Mohon periksa kembali! Input Anda ada yang belum di isi !</div>";
				$cek = "kosong";
			} else {
				...
				if($status_call == 'Retention' || $status_call == 'Profilling') {
					// $sql="INSERT INTO app_tam_hvc (tgl,login,nama_dm,tlp,`status`,reg,fastel,penerima,jenis,kategori,reason,Ket,follow,upd_qco,valid,input,add_on_tsel,prof_agama,prof_facebook,nama_agent,area) VALUES ('$tgl','$login','$nama_dm','$tlp','$status','$reg','$fastel','$penerima','$jenis','$kategori','$reason','$kets','$fol','$upd_qco','Not Valid','Recall','$add','$chanel','$sub_chanel','$nama','$area')";
					$sql = "INSERT INTO app_tam_hvc (tgl,login,nama_agent,area,no_telp,no_inet,ncli,nama_plg,kelamin,hp_utama,layanan,symptom,kategori_hvc,nama_pemilik,relasi,hp_utama2,hp_lain,email,alamat,kota,
									saran,kategori,sub_kat,status_call,keterangan,kode_voucher,tgl_in_voucher,nama_voucher,kendala,follow,valid,input,channel,opt_in,jenis,reason,selisih_harga,upd_qco) 
									VALUES 
									('$tgl','$login','$nama','$area','$no_telp','$no_inet','$ncli','$nama_plg','$kelamin','$hp_utama','$layanan','$symptomp','$kategori_hvc','$nama_pemilik','$relasi','$hp_utama2','$hp_lain',
									'$email','$alamat','$kota','$saran','$kategori','$sub_kat','$status_call','$ket','$kode_voucher','$tgl_in_voucher','$nama_voucher','$kendala','2','Not Valid','Recall','$channel','$hobbyFav','$jenis_call','$reason_call','$convertRupiah','$login_qco')";
	
					$query = mysqli_query($con,$sql);
				}
				else {
					// $sql="INSERT INTO app_tam_hvc (tgl,login,nama_dm,tlp,`status`,reg,fastel,penerima,jenis,kategori,reason,Ket,follow,upd_qco,valid,input,add_on_tsel,prof_agama,prof_facebook,nama_agent,area) VALUES ('$tgl','$login','$nama_dm','$tlp','$status','$reg','$fastel','$penerima','$jenis','$kategori','$reason','$kets','$fol','$login_qco','Not Valid','Recall','$add','$chanel','$sub_chanel','$nama','$area')";
					$sql = "INSERT INTO app_tam_hvc (tgl,login,nama_agent,area,no_telp,no_inet,ncli,nama_plg,kelamin,hp_utama,layanan,symptom,kategori_hvc,nama_pemilik,relasi,hp_utama2,hp_lain,email,alamat,kota,
									saran,kategori,sub_kat,status_call,keterangan,kode_voucher,tgl_in_voucher,nama_voucher,kendala,follow,valid,input,channel,opt_in,jenis,reason,selisih_harga,upd_qco) 
									VALUES 
									('$tgl','$login','$nama','$area','$no_telp','$no_inet','$ncli','$nama_plg','$kelamin','$hp_utama','$layanan','$symptomp','$kategori_hvc','$nama_pemilik','$relasi','$hp_utama2','$hp_lain',
									'$email','$alamat','$kota','$saran','$kategori','$sub_kat','$status_call','$ket','$kode_voucher','$tgl_in_voucher','$nama_voucher','$kendala','$fol','Not Valid','Recall','$channel','$hobbyFav','$jenis_call','$reason_call','$convertRupiah','$login_qco')";
				...
				<div class="row">
					<div class="col-md-12">
						<div class="card card-plain">
							<div class="header">
								<h4 class="title">CWC <small>Form Input Recall HVC <?php echo $nama; ?></small></h4>	
							</div>
							<form id='input' name="demoform" action='form_recall_hvc.php?id=<?php echo "$id";?>' method='post' accept-charset='UTF-8'>
							<div>
							...
							<div class="row">
								...
								<div class="col-md-4">
									<div class="form-group">
										...
										<input class="form-control" type="email" name="email" value="<?php echo $data['email']; ?>" placeholder="Masukkan Email Utama" required />
									</div>
								</div>
							</div>
							<div class="row">
								<div class="col-md-4">
									<div class="form-group">
										...
										<input class="form-control" type="text" name="alamat" value="<?php echo $data['alamat']; ?>" placeholder="Masukkan Alamat" required  />
									</div>
								</div>
								<div class="col-md-4">
									<div class="form-group">
										<label class="control-label" style="color:#008B8B">Kota</label>
										<input class="form-control" type="text" name="kota" value="<?php echo $data['kota']; ?>" placeholder="Masukkan Kota" required  />
									</div>
								</div>
								<div class="col-md-4">
									<div class="form-group">
						...
						<input type="submit" name="Save" class="btn btn-danger" value="Approve">
