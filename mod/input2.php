...
if (isset($_POST['Save'])) {
	...
	// print_r($kategori);
		...
		if ($fastel == ""  or $login == "" or $nama_dm == "" or $tlp == "" or $reg == "" or $status == "" or $jenis == "" or $kategori == "" or $reason == "" or $ket == "" or $follow == "" or $convertRupiah == "" or $chanel == "" or $sub_chanel == "") {
			// echo "<div class=\"alert alert-danger\">Mohon periksa kembali! Input Anda ada yang belum di isi !</div>";
			$cek = "kosong";
		} else {
			if($kategori == "Agree"){
				$sql = "INSERT INTO app_tam_data2 (tgl,login,nama_dm,tlp,`status`,reg,fastel,penerima,jenis,kategori,reason,Ket,follow,valid,input,add_on_tsel,prof_agama,prof_facebook,upd_qco) VALUES ('$tgl','$login','$nama_dm','$tlp','$status','$reg','$fastel','$penerima','$jenis','$kategori','$reason','$ket','$follow','Not Valid','New','$convertRupiah','$chanel','$sub_chanel','$login_qco')";
				$query = mysqli_query($con, $sql);
				$cek = 'sukses';
			}
				...
				$sql = "INSERT INTO app_tam_data2 (tgl,login,nama_dm,tlp,`status`,reg,fastel,penerima,jenis,kategori,reason,Ket,follow,valid,input,add_on_tsel,prof_agama,prof_facebook) VALUES ('$tgl','$login','$nama_dm','$tlp','$status','$reg','$fastel','$penerima','$jenis','$kategori','$reason','$ket','$follow','Not Valid','New','$convertRupiah','$chanel','$sub_chanel')";
				$query = mysqli_query($con, $sql);
				$cek = 'sukses';
			//echo $sql;
			}
		...
		<div class="row">
			<div class="col-md-12">
				<div class="card card-plain">
					...
					<form id='form1' name="demoform" action='input2.php' method='post' accept-charset='UTF-8'>
