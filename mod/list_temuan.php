									...
									<div class="card ">
										...
										<div style="overflow-x:auto;">
											<table id="data-table" width="170px" class="table table-hover table-striped">
												...
												<tbody>
													...
													if ((!(isset($Login))) or ($Login == "")) {
														$query = "SELECT id,tgl,login,nama_agent,area,fastel,nama_dm,tlp,jenis,`status`,valid,upd_qco,prof_hoby,kategori,reason,ket FROM
													app_tam_data2 WHERE tgl BETWEEN '$tgl 00:00:00' AND '$tgl 23:59:59' AND valid='Valid' AND `status`='Contacted' AND area='$area' ORDER BY tgl DESC LIMIT $offset, $dataPerPage";
													} else {
														$query = "SELECT id,tgl,login,nama_agent,area,fastel,nama_dm,tlp,jenis,`status`,valid,upd_qco,prof_hoby,kategori,reason,ket FROM
													app_tam_data2 WHERE tgl BETWEEN '$tgl 00:00:00' AND '$tgl 23:59:59' AND valid='Valid' AND `status`='Contacted' AND area='$area' AND a.login='$Login' ORDER BY a.tgl DESC LIMIT $offset, $dataPerPage";
													}
													//echo "$query";
													$Page = $Page + 1;
															...
															<form action="list_temuan.php?area=<?php echo $area;?>&tanggal=<?php echo $tgl;?>" method="POST" id="FormRekom_qa">
																<div class="modal-body">
																	...
																	?>
																		...
																		<input type="hidden" class="form-control" name="nama_dm" id="nama_dm" value="<?php echo $nama_dm; ?>" required>
											...
											<table>
												<?php
												if ((!(isset($Login))) or ($Login == "")) {
													$query   = "SELECT COUNT(*) AS jumData FROM app_tam_data2 WHERE tgl BETWEEN '$tgl 00:00:00' AND '$tgl 23:59:59'  AND valid='Valid' AND `status`='Contacted' AND area='$area'";
												} else {
													$query   = "SELECT COUNT(*) AS jumData FROM app_tam_data2 WHERE tgl BETWEEN '$tgl 00:00:00' AND '$tgl 23:59:59'  AND valid='Valid' AND `status`='Contacted' AND area='$area' AND login='$Login'";
												}
