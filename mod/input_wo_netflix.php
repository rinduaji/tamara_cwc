	...
	if ($status == "Not Contacted" || $status == "Follow Up Recall") {
		if ($fastel == ""  or $login == "" or $nama_dm == "" or $tlp == "" or $reg == "" or $status == "" or $jenis == "" or $kategori == "" or $ket == "" ) {
			...
			$sql = "INSERT INTO app_tam_data2 (tgl,login,fastel,nama_dm,tlp,reg,jenis,`status`,kategori,ket,follow,valid,input,
					nama_agent,area,upd_qco,speed_loyalty, hssp_type, recom_speed, blast, speed_tertinggi, status_hd) VALUES ('$tgl','$login','$fastel','$nama_dm','$tlp','$reg','$jenis','$status','$kategori','$ket',
					'$follow','Not Valid','New','$nama','$area','$login_qco','$speed_loyalty','$hssp_type','$recom_speed','$blast','$speed_tertinggi','0')";
			//echo $sql;
		...
		if ($fastel == ""  or $login == "" or $nama_dm == "" or $tlp == "" or $reg == "" or $status == "" or $jenis == "" or $kategori == "" or $reason == "" or $ket == "" or $follow == "" or $penerima == "") {
			...
			if($kategori == "Agree"){
				$sql = "INSERT INTO app_tam_data2 (tgl,login,nama_dm,tlp,`status`,reg,fastel,penerima,jenis,kategori,reason,Ket,follow,valid,input,add_on_tsel,upd_qco,nama_agent,area,
						paket_steaming,speed_asal,speed_tujuan,`param`,internet,ott1,ott2,paket_harga,diskon,total_harga_diskon,speed_loyalty, hssp_type, recom_speed, blast, speed_tertinggi, status_hd) VALUES ('$tgl','$login','$nama_dm','$tlp','$status','$reg','$fastel','$penerima','$jenis','$kategori',
						'$reason','$ket','$follow','Not Valid','New','$add','$login_qco','$nama','$area','$paket_streaming','$speed_asal','$speed_tujuan','$param','$internet','$ott1',
						'$ott2','$paket_harga','$diskon','$total_harga_diskon','$speed_loyalty','$hssp_type','$recom_speed','$blast','$speed_tertinggi','0')";
				$query = mysqli_query($con, $sql);
				// print_r($sql);
				// die();
				$cek = 'sukses';
				...
				$sql = "INSERT INTO app_tam_data2 (tgl,login,nama_dm,tlp,`status`,reg,fastel,penerima,jenis,kategori,reason,Ket,follow,valid,input,add_on_tsel,nama_agent,area,
						paket_steaming,speed_asal,speed_tujuan,`param`,internet,ott1,ott2,paket_harga,diskon,total_harga_diskon,speed_loyalty, hssp_type, recom_speed, blast, speed_tertinggi, status_hd) VALUES ('$tgl','$login','$nama_dm','$tlp','$status','$reg','$fastel','$penerima','$jenis','$kategori',
						'$reason','$ket','$follow','Not Valid','New','$add','$nama','$area','$paket_streaming','$speed_asal','$speed_tujuan','$param','$internet','$ott1','$ott2',
						'$paket_harga','$diskon','$total_harga_diskon','$speed_loyalty','$hssp_type','$recom_speed','$blast','$speed_tertinggi','0')";
				$query = mysqli_query($con, $sql);
				$cek = 'sukses';
			//echo $sql;
				$data = [
			...
			if($kategori == "Not Agree" AND $reason == "RNA") {
				...
				$query_update_master = "UPDATE app_wo_netflix_master SET `status` = '1', lup = '$tgl' WHERE fastel='$fastel'";
				$result_update_master = mysqli_query($con, $query_update_master);
			}
							...
							<div class="row">
								...
								<div class="col-md-4">
									<div class="form-group">
										...
										<input class="form-control" type="text" name="blast" id="blast" value="<?= (($data_wo['blast'] == '0') ? 'BELUM DI BLAST' : 'SUDAH DI BLAST') ?>" placeholder="Blast" required  readonly/>
									</div>
								</div>
								<div class="col-md-4">
									<div class="form-group">
