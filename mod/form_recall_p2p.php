		...
		if($status=="Not Contacted" || $status=="NOT CONTACTEBLE")
			{   
				if ($init_offer == ""  or $label == "" or $msisdn == "" or $perner == "" or $keterangan == "" ) {
					...
					$sql1="UPDATE `app_tam_p2p` SET `follow`='1'  WHERE (`id`='$id')";
					$query=mysqli_query($con,$sql1);
					
					// $sql="INSERT INTO app_tam_hvc (tgl,login,nama_dm,tlp,`status`,reg,fastel,penerima,jenis,kategori,reason,Ket,follow,upd_qco,valid,input,add_on_tsel,nama_agent,area) VALUES ('$tgl','$username','$nama_dm','$tlp','$status','$reg','$fastel','$penerima','$jenis','$kategori','$reason','$kets','5','$upd_qco','Not Valid','$reg','Recall','$add','$nama_agent','$area_agent')";
					...
					'$follow','Recall','Not Valid','$login_qco','$type_p2p')";

					//echo $sql;
					$query = mysqli_query($con,$sql);
					$cek = 'sukses';
					// if ($status == "Not Contacted") {
						$sql_update_status_halo = "UPDATE app_wo_p2p SET status='0', lup='$tgl', pilih_login = '$username' WHERE msisdn='$msisdn'";
						mysqli_query($con, $sql_update_status_halo);
					// } elseif ($status == "Follow Up") {
					// 	$sql_update_status_halo = "UPDATE app_wo_p2p SET status='1', lup='$tgl' WHERE no_telp='$no_telp'";
					// 	mysqli_query($con, $sql_update_status_halo);
					// }
					clearstatcache();
                    if($type_p2p == "REGULER") {
						$location = "list_recall_p2p_reguler.php";
					}
                ...
                if($kategori == "Agree"){
                    ...
                    if ($init_offer == "" AND $label == "" AND $msisdn == "" AND $nama_plg == "" AND $ktp == "" AND $kk == "" AND $no_hp_lain == "" AND $tgl_lahir == "" AND 
                        ...
                        $sql = "INSERT INTO app_tam_p2p 
                                    ...
                                    '$alamat_ktp_plg','$alamat_domisili','$kode_pos','$no_cp_ref','$nama_cp_ref','$tgl_setuju','$jenis','$status','$kategori','$reason','$keterangan',
                                    '$follow','Recall','Not Valid','$revenue','$login_qco','$type_p2p','$cls')";
				...
				<div class="row">
					<div class="col-md-12">
						<div class="card card-plain">
							<div class="header">
								<h4 class="title">CWC <small>Form Input Recall P2P <?php echo $nama; ?></small></h4>	
							</div>
							<form id='input' name="demoform" action='form_recall_p2p.php?id=<?php echo "$id";?>' method='post' accept-charset='UTF-8'>
							<div>
