		...
		if($status_call=="Not Contacted" || $status_call=="Follow Up")
			{   
				if ($no_telp == "" or $jenis_call == "" or $kategori == "" or $status_call == "" ) {
					...
					// $sql="INSERT INTO app_tam_hvc (tgl,login,nama_dm,tlp,`status`,reg,fastel,penerima,jenis,kategori,reason,Ket,follow,upd_qco,valid,input,add_on_tsel,nama_agent,area) VALUES ('$tgl','$login','$nama_dm','$tlp','$status','$reg','$fastel','$penerima','$jenis','$kategori','$reason','$kets','5','$upd_qco','Not Valid','$reg','Recall','$add','$nama','$area')";
					$sql = "INSERT INTO app_tam_halo (tgl,login,nama_agent,area,no_telp,no_ktp,tgl_setuju,no_kk,alamat_kirim,paket_halo,email,alamat,kategori,keterangan,follow,jenis,`status`,reason,fastel,nama_plg,input,kota,valid,reg) 
							VALUES 
							('$tgl','$login','$nama','$area','$no_telp','$no_ktp','$tgl_setuju','$no_kk','$alamat_kirim','$paket_halo','$email','$alamat','$kategori','$keterangan','$follow',
							'$jenis_call','$status_call','$reason_call','$fastel','$nama_plg','Recall','$kota','Not Valid','$reg')";

					//echo $sql;
					$query = mysqli_query($con,$sql);
					$cek = 'sukses';
					if ($status_call == "Not Contacted") {
						$sql_update_status_halo = "UPDATE app_wo_halo SET status='0', lup='$tgl', pilih_login = '$login' WHERE no_telp='$no_telp'";
						mysqli_query($con, $sql_update_status_halo);
					} elseif ($status_call == "Follow Up") {
						$sql_update_status_halo = "UPDATE app_wo_halo SET status='1', lup='$tgl', pilih_login = '$login' WHERE no_telp='$no_telp'";
						mysqli_query($con, $sql_update_status_halo);
					}
					clearstatcache();
					$location="list_recall_halo.php";		
			...
			if ($no_telp == "" or $jenis_call == "" or  $reason_call == "" or $kategori == "" or $status_call == "" ) {
                ...
                if($kategori == "Agree"){
                    ...
                    if ($no_telp == "" or $msisdn == "" or $jenis_call == "" or $kategori == "" or $status_call == "" or $no_ktp == "" or $no_kk == "" or $alamat == "" or $alamat_kirim == "" or $email == "" 
                        ...
                        follow,jenis,`status`,reason,fastel,nama_plg,input,kota,upd_qco,valid,reg) 
                        VALUES 
                        ('$tgl','$login','$nama','$area','$no_telp','$no_ktp','$msisdn','$reserve_id','$area_tsel','$tgl_setuju','$no_kk','$alamat_kirim','$paket_halo','$email','$alamat','$kategori','$keterangan','2',
                        '$jenis_call','$status_call','$reason_call','$fastel','$nama_plg','Recall','$kota','$login_qco','Not Valid','$reg')";
                            ...
                            $sql_update_no_halo = "UPDATE app_no_halo SET status_nomer = '1', tgl_pickup='$date', pilih_login = '$login' WHERE no_halo='$msisdn'";
                    ...
                    $sql = "INSERT INTO app_tam_halo (tgl,login,nama_agent,area,no_telp,no_ktp,tgl_setuju,no_kk,alamat_kirim,paket_halo,email,alamat,kategori,keterangan,follow,jenis,`status`,reason,fastel,nama_plg,input,kota,valid,reg) 
                    VALUES 
                    ('$tgl','$login','$nama','$area','$no_telp','$no_ktp','$tgl_setuju','$no_kk','$alamat_kirim','$paket_halo','$email','$alamat','$kategori','$keterangan','8',
                    '$jenis_call','$status_call','$reason_call','$fastel','$nama_plg','Recall','$kota','Not Valid','$reg')";
    
                    $query = mysqli_query($con, $sql);
                    $cek = 'sukses';
    
                    $sql_update_status_halo = "UPDATE app_wo_halo SET status='1', lup='$tgl', pilih_login = '$login' WHERE no_telp='$msisdn'";
                    mysqli_query($con, $sql_update_status_halo);
                    ...
                    $sql = "INSERT INTO app_tam_halo (tgl,login,nama_agent,area,no_telp,no_ktp,tgl_setuju,no_kk,alamat_kirim,paket_halo,email,alamat,kategori,keterangan,follow,jenis,`status`,reason,fastel,nama_plg,input,kota,valid,reg) 
                    VALUES 
                    ('$tgl','$login','$nama','$area','$no_telp','$no_ktp','$tgl_setuju','$no_kk','$alamat_kirim','$paket_halo','$email','$alamat','$kategori','$keterangan','1',
                    '$jenis_call','$status_call','$reason_call','$fastel','$nama_plg','Recall','$kota','Not Valid','$reg')";
				...
				<div class="row">
					<div class="col-md-12">
						<div class="card card-plain">
							...
							<form id='input' name="demoform" action='form_recall_halo.php?id=<?php echo "$id";?>' method='post' accept-charset='UTF-8'>
							<div>
							...
							<div class="row">
														...
														<div class="form-group">
															...
															<select id="js-example-basic-multiple" name="msisdn" class="form-control" onChange="document.demoform.submit()" readonly>
																...
																<?php
																	...
																	$whr_area = '';
																	if($data['reg'] == 1) {
																		$whr_area = " AND area_tsel = 'Area 1' ";
																	}
																	else if($data['reg'] == 2 || $data['reg'] == 3) {
																		$whr_area = " AND area_tsel = 'Area 2' ";
																	}
																	else if($data['reg'] == 4 || $data['reg'] == 5) {
																		$whr_area = " AND area_tsel = 'Area 3' ";
																	}
																	else if($data['reg'] == 6 || $data['reg'] == 7) {
																		$whr_area = " AND area_tsel = 'Area 4' ";
																	}

																	$qsk = mysqli_query($con, "SELECT * FROM app_no_halo WHERE status_nomer = 1 $whr_area ORDER BY no_halo ASC");
								...
								<div class="col-md-12">
									<div class="form-group">
										...
										<textarea class="form-control" id="keterangan" name="keterangan" rows="5" value="<?=($keterangan == NULL) ? $data['keterangan'] : $keterangan; ?>"></textarea>
									</div>
								</div>
							</div>
